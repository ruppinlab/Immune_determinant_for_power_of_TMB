...
setwd('/Users/sinhas8/Downloads/fda-tmbCriteria-Power-vs-age-sex/Data/')
```
<!-- Load & Preprocess data -->
```{r}
setwd('/Users/sinhas8/approve_tmbCriteria_vs_age_Sex/Tools/fda-tmbCriteria-Power-vs-age-sex/Data/')
...
create_corr_with_Immune_factors<- function(infunc_cancer_types=allcancer_types,
  ...
  infunc_cancer_types_filtered=intersect(rownames(infun_Tmb_Resp_Association), infunc_cancer_types)
  features_resp= infunc_immune_matrix[na.omit(match(infunc_cancer_types_filtered,
                                                    rownames(infunc_immune_matrix))),]
  target_resp= infun_Tmb_Resp_Association[rownames(features_resp),'effect_size']
  cor.test(features_resp[,2],target_resp)
  correlation_matrix= data.frame(t(apply(features_resp, 2, function(x)
  ...
  # print(correlation_matrix[order(abs(correlation_matrix[,2]), decreasing = T),])
    ...
    labs(x='Immune Factors', y=paste('Correlation Rho with ', response_type,'-TMB', ' HR' ,sep='' ))+
    ggtitle(infunc_title)
  ggsave(paste('/Users/sinhas8/approve_tmbCriteria_vs_age_Sex/Results_Figures/tmb-responseVSTME_factors',
               consider_cancer_type,'_',
               response_type,'.tiff', sep ='' ),
...
create_scatter_plot<- function(infunc_cancer_types=allcancer_types,
  ...
  Panel3 <- ggplot(df2plot, aes(x=rank(effect_size), y=rank(features_resp), size= -log(P, 10) )) + 
    ...
    labs(x='Hazard Ratio (rank)',
         y=paste(gsub('\\.',' ',immune_feature_of_interest),  '(rank)'), size='-log10(P)')+
    stat_cor(label.y = 0, label.x = 3, method = 'p', cor.coef.name = 'rho', size=6)+
    geom_label_repel(aes(label=rownames(df2plot), size=0.5))+
    coord_flip()
  ggsave(paste('/Users/sinhas8/approve_tmbCriteria_vs_age_Sex/Results_Figures/tmb-responseVS',
