...
Fig_ORR_Only_Sig_Tmb.Resp_associated_cancerTypes=create_corr_with_Immune_factors(
  ...
  infunc_title = 'Cancer types with Sig tmb-ORR odds ratio')

combined_ORR=ggarrange(Fig_ORR_allcancer_types[[2]], 
                       Fig_ORR_Only_Sig_Tmb.Resp_associated_cancerTypes[[2]], 
                       common.legend = T)
ggsave(paste('/Users/sinhas8/approve_tmbCriteria_vs_age_Sex/Results_Figures/Fig1A_tmb-ORR_associationVSTME_factors',
             '.pdf', sep ='' ),
...
combined_OS=ggarrange(Fig_OS_allcancer_types[[2]], 
                       ...
                       common.legend = T)
ggsave(paste('/Users/sinhas8/approve_tmbCriteria_vs_age_Sex/Results_Figures/Fig1B_tmb-OS_associationVSTME_factors',
             '.pdf', sep ='' ),
...
ggsave(paste('/Users/sinhas8/approve_tmbCriteria_vs_age_Sex/Results_Figures/Fig1C_tmb-PFS_associationVSTME_factors',
             '.pdf', sep ='' ),
...
mean(unlist(df_all_cor[df_all_cor$features=='NTL',-1]))
...
top_Modulators=data.frame(Modulators=head(features_ranked, 5),
                          PFS=ceiling(rowMeans(ranked_score_top_mods[,1:2])),
                          OS=ceiling(rowMeans(ranked_score_top_mods[,3:4])),
                          ORR=ceiling(rowMeans(ranked_score_top_mods[,5:6])),
                          mean_corr= )
write.csv(top_Modulators,'/Users/sinhas8/approve_tmbCriteria_vs_age_Sex/Data/top_Modulators_Mar1.csv')
```
...
MeanCorr_df$mod_labels[match(head(features_ranked, 5), MeanCorr_df$Modulators)]=head(features_ranked, 5)

panle4=ggplot(MeanCorr_df, aes(x=ORR, y=durable_response, label=mod_labels))+
  geom_point(aes(color= mod_labels!=''))+
  geom_label_repel(min.segment.length = 1)+
  theme_bw(base_size = 12)+
  labs(x='Correlation strength with Odds ratios of objective\nresponse rate in TMB-high vs low patients',
       y='Correlation strength with Hazard ratios of durable\nresponse (OS/PFS) in TMB-high vs low patients')+
  theme(legend.position = 'none')
  
ggsave(paste('/Users/sinhas8/approve_tmbCriteria_vs_age_Sex/Results_Figures/Panel4.tiff', sep ='' ),
       panle4, height=6, width = 6, units = 'in', dpi = 500)
